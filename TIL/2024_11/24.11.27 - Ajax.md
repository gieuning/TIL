...
서버는 어떠한 경우라도 그냥 결과를 주지 않음
요청을 해야 결과를 줌 - 폴링
푸시는 실시간 처리가 가능
get 방식에는 무조건 인코딩 할 것  (영문자, 숫자 제외)

httpReq.send(null); - get 방식
httpReq.send(query); - post 방식

xml에는 대소문자를 구분하고 최상위 root가 존재해야한다.
...
XMLHttpRequset 객체를 생성하여 ex06_ok.jsp에 요청을 보낸다
` httpReq.responseType = 'json';` 을 설정하면, 서버 응답이 JSON으로 자동 변환

ex06_ok.jsp는 JSONObject와 JSONArray를 사용해 JSON 데이터를 생성하고,
out.print(job.toString());으로 응답을 보낸다.
이 응답이 ex06으로 전달
ex06에서는 httpReq.response를 통해 json 객체로 바로 접근 가능 




// responseType='json' 으로 설정한 경우 response로 받는다.
let job = httpReq.response; // JSON 객체